...
type UrgencyFilter = 'All' | 'Critical' | 'High' | 'Medium' | 'Low' | 'contacted'
...
const FILTER_PILLS: { label: string; value: UrgencyFilter; cls: string; activeCls: string }[] = [
  { label: 'All', value: 'All', cls: 'border-[#E5E7EB] bg-white text-[#6B7280]', activeCls: 'bg-[#028090] text-white border-[#028090]' },
  { label: 'Critical', value: 'Critical', cls: 'border-[#E5E7EB] bg-white text-[#6B7280]', activeCls: 'bg-[#DC2626] text-white border-[#DC2626]' },
  { label: 'High', value: 'High', cls: 'border-[#E5E7EB] bg-white text-[#6B7280]', activeCls: 'bg-[#D97706] text-white border-[#D97706]' },
  { label: 'Medium', value: 'Medium', cls: 'border-[#E5E7EB] bg-white text-[#6B7280]', activeCls: 'bg-[#2563EB] text-white border-[#2563EB]' },
  { label: 'Low', value: 'Low', cls: 'border-[#E5E7EB] bg-white text-[#6B7280]', activeCls: 'bg-[#64748B] text-white border-[#64748B]' },
  { label: 'Contacted', value: 'contacted', cls: 'border-[#E5E7EB] bg-white text-[#6B7280]', activeCls: 'bg-[#475569] text-white border-[#475569]' },
]

const FILTER_BANNERS: Record<string, { bg: string; border: string; color: string; icon: React.ReactNode; text: string }> = {
  Critical: {
    bg: 'bg-[#FEF2F2]', border: 'border-[#FECACA]', color: 'text-[#991B1B]',
    icon: <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1.5L13 12.5H1L7 1.5z" stroke="#DC2626" strokeWidth="1.4" strokeLinejoin="round"/><path d="M7 5.5v3" stroke="#DC2626" strokeWidth="1.4" strokeLinecap="round"/><circle cx="7" cy="10.5" r=".7" fill="#DC2626"/></svg>,
    text: 'these patients have not been seen in 2+ years and need immediate outreach.',
  },
  High: {
    bg: 'bg-[#FFFBEB]', border: 'border-[#FDE68A]', color: 'text-[#92400E]',
    icon: <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 2v9M3.5 5.5L7 2l3.5 3.5" stroke="#D97706" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>,
    text: 'these patients are significantly overdue and should be contacted soon.',
  },
  Medium: {
    bg: 'bg-[#EFF6FF]', border: 'border-[#BFDBFE]', color: 'text-[#1E40AF]',
    icon: <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 7h10" stroke="#2563EB" strokeWidth="1.4" strokeLinecap="round"/></svg>,
    text: 'these patients are moderately overdue for their well visit.',
  },
  Low: {
    bg: 'bg-[#F8FAFC]', border: 'border-[#CBD5E1]', color: 'text-[#475569]',
    icon: <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 2v9M3.5 8.5L7 12l3.5-3.5" stroke="#64748B" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>,
    text: 'these patients are slightly overdue and can be scheduled at next availability.',
  },
}
...
const TABLE_BORDER: Record<string, string> = {
  ...
  Low: 'border-[#CBD5E1]',
  contacted: 'border-[#CBD5E1]',
}

const PAGE_ACTIVE_BG: Record<string, string> = {
  Critical: 'bg-[#DC2626] border-[#DC2626]',
  High: 'bg-[#D97706] border-[#D97706]',
  Medium: 'bg-[#2563EB] border-[#2563EB]',
  Low: 'bg-[#64748B] border-[#64748B]',
  All: 'bg-[#028090] border-[#028090]',
  contacted: 'bg-[#475569] border-[#475569]',
}

const DAYS_COUNTS: Record<string, number> = {
  Critical: 14, High: 28, Medium: 19, Low: 11,
}
...
export default function AlertsClient() {
  ...
  const urlFilter = (searchParams.get('filter') as UrgencyFilter) || 'All'
  ...
  const [sortOrder, setSortOrder] = useState<'days_desc' | 'days_asc'>('days_desc')
  ...
  // Derive the selected patient from the live patients array so it reflects updates
  ...
  function setFilter(f: UrgencyFilter) {
    setSelectedRows(new Set())
    if (f === 'All') router.push('/alerts')
    else router.push(`/alerts?filter=${f}`)
  }

  const contactedCount = patients.filter((p) => p.outreach_status === 'contacted').length

  const filtered = !loading ? patients
    .filter((p) => {
      const matchesSearch =
        search === '' ||
      ...
      const matchesFilter =
        urlFilter === 'All' ? true
        : urlFilter === 'contacted' ? p.outreach_status === 'contacted'
        : p.urgency_label === urlFilter
      return matchesSearch && matchesFilter
    })
    .sort((a, b) => sortOrder === 'days_desc' ? b.days_overdue - a.days_overdue : a.days_overdue - b.days_overdue)
  : []
  ...
  const banner = urlFilter !== 'All' ? FILTER_BANNERS[urlFilter] : null
  const tableBorderClass = urlFilter !== 'All' ? TABLE_BORDER[urlFilter] : 'border-[#e2e8f0]'
  ...
  return (
    <div className="p-4 md:p-7 flex flex-col gap-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          {urlFilter !== 'All' && (
            <div className="flex items-center gap-1.5 text-[12px] text-[#9CA3AF] mb-1.5">
              <button onClick={() => setFilter('All')} className="text-[#6B7280] hover:text-[#111] cursor-pointer">Patient Alerts</button>
              <span className="text-[#D1D5DB]">›</span>
              <span className="text-[#111] font-medium">{urlFilter}</span>
            </div>
          )}
          <h1 className="text-[26px] font-semibold text-[#111] tracking-tight">Patient Alerts</h1>
        </div>
        {urlFilter !== 'All' && (
          <button
            onClick={() => setFilter('All')}
            className="flex items-center gap-1.5 text-[12px] text-[#6B7280] px-3 py-1.5 rounded-md border border-[#E5E7EB] bg-white hover:bg-[#f0f9ff] transition-colors mt-1.5"
          >
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2 2l8 8M10 2l-8 8" stroke="#6B7280" strokeWidth="1.4" strokeLinecap="round"/></svg>
            Clear filter
          </button>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2 border border-[#E5E7EB] rounded-[4px] bg-white px-3 py-2 text-[13px] text-[#9CA3AF] w-full md:w-72">
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="6" cy="6" r="4" stroke="#9CA3AF" strokeWidth="1.3"/><path d="M10 10l2.5 2.5" stroke="#9CA3AF" strokeWidth="1.3" strokeLinecap="round"/></svg>
          <input
            type="text"
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 flex-nowrap flex-1 -mb-0.5">
            {FILTER_PILLS.map(({ label, value, cls, activeCls }) => {
              const isActive = urlFilter === value
              const count = loading ? null
                : value === 'All' ? patients.length
                : value === 'contacted' ? contactedCount
                : patients.filter((p) => p.urgency_label === value).length
              return (
                <button
                  key={value}
                  onClick={() => setFilter(value)}
                  className={`text-[12px] px-3.5 py-1.5 rounded-full border font-medium whitespace-nowrap transition-colors cursor-pointer shrink-0 ${isActive ? activeCls : cls}`}
                >
          ...
          <select
            ...
            className="shrink-0 rounded-[4px] border border-[#E5E7EB] px-2.5 py-1.5 text-[12px] bg-white text-[#374151] cursor-pointer"
          >
            <option value="days_desc">Sort: Days overdue ↓</option>
            <option value="days_asc">Sort: Days overdue ↑</option>
          </select>
        </div>
      </div>
      ...
      {banner && (
        <div className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-[8px] border text-[12px] font-medium ${banner.bg} ${banner.border} ${banner.color}`}>
          {banner.icon}
          Showing <strong className="font-mono font-semibold mx-1">{filtered.length} {urlFilter} patients</strong> — {banner.text}
        </div>
      )}

      {/* Table */}
      <div className={`bg-white rounded-[6px] border overflow-hidden ${tableBorderClass}`} style={{ borderWidth: urlFilter !== 'All' ? '1.5px' : '1px' }}>
        <div className="flex items-center justify-between px-[18px] py-[11px] border-b border-[#F3F4F6]">
          <div className="text-[12px] text-[#6B7280]">
            ...
            {urlFilter === 'All' ? ' overdue for their well visit' : urlFilter === 'contacted' ? ' with contacted outreach status' : ` with ${urlFilter} urgency`}
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-2 text-[#9CA3AF]">
            ...
            <span className="text-[13px]">No patients match your filters.</span>
          </div>
        ) : (
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#f0f9ff]">
                ...
                {['Patient', 'Age', 'Days Overdue', 'Last Well Visit', 'Urgency', 'Action', ''].map((h) => (
                  <th key={h} className={[
                    ...
                    (h === 'Age' || h === 'Last Well Visit' || h === 'Action' || h === '') ? 'hidden md:table-cell' : '',
                  ].filter(Boolean).join(' ')}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
        ...
        {!loading && filtered.length > 0 && (
          <div className="flex items-center justify-center gap-1 py-4">
            <button className="w-[34px] h-[34px] rounded-[4px] border border-[#E5E7EB] bg-white text-[13px] text-[#6B7280] flex items-center justify-center hover:bg-[#f0f9ff]">‹</button>
            <button className={`w-[34px] h-[34px] rounded-[4px] border text-[13px] font-medium flex items-center justify-center ${PAGE_ACTIVE_BG[urlFilter] || 'bg-[#028090] border-[#028090]'} text-white`}>1</button>
            <button className="w-[34px] h-[34px] rounded-[4px] border border-[#E5E7EB] bg-white text-[13px] text-[#374151] flex items-center justify-center hover:bg-[#f0f9ff]">2</button>
            <button className="w-[34px] h-[34px] rounded-[4px] border border-[#E5E7EB] bg-white text-[13px] text-[#374151] flex items-center justify-center hover:bg-[#f0f9ff]">›</button>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg rounded-[6px] bg-white p-6 shadow-lg ring-1 ring-[#e2e8f0]">
            <div className="mb-4 flex items-start justify-between gap-3">
              ...
              <button onClick={() => setModal((m) => ({ ...m, open: false }))} className="rounded-[4px] p-1.5 text-[#9CA3AF] hover:bg-[#f0f9ff] hover:text-[#374151] transition-colors">
...
function PatientRow({
  ...
  const urgencyRowBg =
    patient.urgency_label === 'Critical' ? 'bg-[#FFF7F7] hover:bg-[#FFECEA]'
    : patient.urgency_label === 'High' ? 'bg-[#FFFBF4] hover:bg-[#FFF5E9]'
    : 'bg-white hover:bg-[#F8FAFC]'

  const avatarBg =
    activeFilter === 'Critical' ? 'bg-[#FFF5F5] text-[#991B1B]'
    : activeFilter === 'High' ? 'bg-[#FFFBEB] text-[#92400E]'
    : 'bg-[#f0f9ff] text-[#05668d]'
    ...
    >
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-2.5">
          <div className={`w-[34px] h-[34px] rounded-full flex items-center justify-center text-[11px] font-semibold shrink-0 cursor-pointer hover:ring-2 hover:ring-[#028090] hover:ring-offset-1 transition-all ${avatarBg}`}>
            {initials}
          </div>
          <div>
            <div className="text-[13px] font-semibold text-[#111]">{patient.first_name} {patient.last_name}</div>
      ...
      <td className="px-4 py-3 min-w-[260px] hidden md:table-cell" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center justify-center gap-1 w-[100px] whitespace-nowrap text-[11px] font-medium px-2.5 py-[5px] rounded-[5px] border border-[#e2e8f0] bg-[#f0f9ff] text-[#05668d] hover:bg-[#dbeafe] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            ✦ AI script
          </button>
          <button
      ...
      <div className="fixed right-0 top-0 h-full w-full md:w-[380px] z-50 bg-white shadow-md border-l border-[#e2e8f0] flex flex-col overflow-hidden">
        ...
        <div className="px-5 py-4 border-t border-[#F3F4F6] flex flex-col gap-2 shrink-0">
          <button
            onClick={() => onGenerateScript(patient)}
            disabled={isContacted}
            className="w-full flex items-center justify-center gap-1.5 text-[12px] font-medium px-3 py-2.5 rounded-[4px] border border-[#e2e8f0] bg-[#f0f9ff] text-[#05668d] hover:bg-[#dbeafe] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            ✦ Generate AI script
          </button>
          <button
